...
public class LibroVentasView extends JPanel {
    ...
    private void initComponents() {
        ...
        panelTabla.setBorder(BorderFactory.createTitledBorder("Libro de Ventas IVA"));

        String[] columnas = {"N°", "Fecha", "N° Doc", "Tipo", "Cliente", "NRC",
                "Exentas", "Gravadas", "Débito Fiscal", "Total"};
        ...
        for (int i = 6; i <= 9; i++) {
            tablaVentas.getColumnModel().getColumn(i).setCellRenderer(rendererDerecha);
        }

        // Ancho de columnas
        tablaVentas.getColumnModel().getColumn(0).setPreferredWidth(30);
        tablaVentas.getColumnModel().getColumn(4).setPreferredWidth(180);
        ...
        lblTotalDebito = crearLabelTotal("Débito Fiscal: $0.00");
        lblTotalDebito.setForeground(new Color(211, 47, 47));
        lblTotalDebito.setFont(new Font("Arial", Font.BOLD, 14));
        ...
        btnGenerar.addActionListener(e -> {
            if (controller != null) controller.cargarLibroVentas();
        });
        btnExportar.addActionListener(e -> {
            if (controller != null) controller.exportarLibroVentasCSV();
        });
    }
    ...
    public void cargarDatos(List<RegistroLibroVentas> registros) {
        ...
        for (RegistroLibroVentas r : registros) {
            modeloTabla.addRow(new Object[]{
                    ...
                    FORMATO.format(r.getVentasExentas()),
                    FORMATO.format(r.getVentasGravadas()),
                    FORMATO.format(r.getDebitoFiscal()),
                    FORMATO.format(r.getVentaTotal())
            });
        }
    }
    ...
    public void mostrarTotales(Map<String, BigDecimal> totales) {
        ...
        lblTotalDebito.setText("Débito Fiscal: " + FORMATO.format(totales.get("debitoFiscal")));
